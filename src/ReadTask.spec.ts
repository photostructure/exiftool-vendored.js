...
describe("ReadTask", () => {
  // WAT WHY? By running the entire suite with both values of
  // includeImageDataMD5, we ensure consistent behavior for either value.
  for (const includeImageDataMD5 of [true, false]) {
    describe(JSON.stringify({ includeImageDataMD5 }), () => {
      let exiftool: ExifTool
      before(() => (exiftool = new ExifTool({ includeImageDataMD5 })))
      after(() => exiftool.end())
   ...
   */
        ...
        it("ImageDataMD5", async () => {
          const t = await exiftool.read(join(testDir, "with_thumb.jpg"))
          expect(t.ImageDataMD5).to.eql(
            includeImageDataMD5 ? "5617def2642dbd90ab6a2d4f185d7850" : undefined
          )
        })
      ...
      describe("non-alphanumeric filenames", () => {
        for (const { str, desc } of NonAlphaStrings) {
          it("reads with " + desc, async () => {
            ...
            expect(t.DateTimeOriginal?.toString()).to.match(
              /^2016-08-12T13:28:50/
            )
          })
        }
